...
import { removeFloorapi } from "@/app/redux/project_setting/project_CRUD";
...
const Notification: React.FC<NotificationProps> = ({ onClose, languageData }) => {
  ...
  const handleSave = () => {
    if (selected_project) {
      dispatch(removeFloorapi(selected_project.floor_uid));
      onClose();
    } else {
      ...
      <div className="bg-[#D9D9D9] rounded-lg shadow-xl p-6 max-w-md w-full">
        <div className="mb-4 p-4 bg-red-100 border-l-4 border-red-500 text-red-700">
          <p>
            ...
            {selected_project
              ? languageData.message.delete_floor.replace("{floorName}", selected_project.floor_name)
